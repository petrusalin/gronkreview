...
func swiftReviewPrompt() -> String {
    """
    You will be asked to code review a piece of swift code. This will be the INPUT
    There are some CORE PRINCIPLES that should be taken into consideration when reviewing the INPUT
    You will provide the feedback (OUTPUT)
    
    PRINCIPLES:
    The following should be considered highly important when doing the code review:
    ...
    - do not do any view work on view controller init. the views are not loaded yet. it is only safe after load view/in viewDidLoad
    - user data should be secure
    - API keys should not be stored plain in code
    
    OUTPUT:
    Feedback should be provided as comments. Each comment should have the following structure:
    1. Comment text
    2. Original lines of code
    3. Modified lines of code as a result of applying the comments
